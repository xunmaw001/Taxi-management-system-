    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(cheliang !=null){
            ...
            BeanUtils.copyProperties( cheliang , view );//把实体数据重构到view中
                ...
                //级联表
                JiashiyuanEntity jiashiyuan = jiashiyuanService.selectById(cheliang.getJiashiyuanId());
                if(jiashiyuan != null){
                    BeanUtils.copyProperties( jiashiyuan , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
    ...
    */
    ...
    public R save(@RequestBody CheliangEntity cheliang, HttpServletRequest request){
        ...
        if(false)
            ...
            cheliang.setJiashiyuanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<CheliangEntity> queryWrapper = new EntityWrapper<CheliangEntity>()
            .eq("jiashiyuan_id", cheliang.getJiashiyuanId())
...
//                            cheliangEntity.setJiashiyuanId(Integer.valueOf(data.get(0)));   //驾驶员 要改的
//                            cheliangEntity.setCheliangUuidNumber(data.get(0));                    //车辆编号 要改的
//                            cheliangEntity.setCheliangName(data.get(0));                    //车辆名称 要改的
//                            cheliangEntity.setGoumaiTime(sdf.parse(data.get(0)));          //车辆购买日期 要改的
